...
const donorSchema = Joi.object({
  ...
  // FIX: hospital_id is optional — hospital_staff users don't send it,
  //      the controller reads it from req.user.hospital_id instead.
  hospital_id:      Joi.number().integer().positive().optional().allow(null),
...
const recipientSchema = Joi.object({
  ...
  hospital_id:         Joi.number().integer().positive().optional().allow(null), // same fix
