...
export default function Dashboard() {
    ...
    const { get: getAnalytics, data: analyticsData, loading: aLoading } = useApi();
        ...
        <div className="dashboard">
            <div className="dash-main">
                ...
                <div className="dash-header">
                    <div>
                        ...
                        <div className="dash-sub">
                            {aLoading ? 'Refreshing…' : `Updated ${timeAgo(lastUpdated.toISOString())}`}
                            &nbsp;·&nbsp;{new Date().toLocaleDateString('en-IN', { weekday: 'long', day: '2-digit', month: 'short', year: 'numeric' })}
                        </div>
                    </div>
                    <div className="dash-actions">
                        <button className="btn-secondary" style={{ padding: '8px 16px', fontSize: 12 }} onClick={() => navigate('/donors/register')}>+ Register Donor</button>
                        <button className="btn-primary" style={{ padding: '8px 16px', fontSize: 12 }} onClick={() => navigate('/matching')}>Run Match</button>
                    </div>
                </div>
                ...
                <div className="kpi-grid">
                    ...
                    <KPICard label="Recipients Waiting" value={stats.recipients_waiting ?? '—'} delta="On national waiting list" color="blue" barPct={Math.min(100, (stats.recipients_waiting || 0) / 500 * 100)} />
                    <KPICard label="Total Transplants" value={stats.total_transplants ?? '—'} delta="All time" color="purple" barPct={60} />
                        ...
                        {organs.slice(0, 8).map(o => (
                            <div key={o.organ_id} className="match-item" onClick={() => navigate(`/matching/${o.organ_id}`)}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
                                    ...
                                    <div style={{ fontSize: 10, color: 'var(--text-2)' }}>{o.hours_remaining != null ? `${Number(o.hours_remaining).toFixed(1)}h left` : '—'}</div>
                                </div>
                                <div className="timer-chip" style={{ background: 'var(--teal-dim)', color: 'var(--teal)' }}>Match →</div>
                            </div>
                        ))}
                        {organs.length > 8 && (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        <div className="panel">
                            ...
                            <div className="viability-col">
                                ...
                                {organs.length === 0 && <div style={{ padding: '12px 0', color: 'var(--text-2)', fontSize: 12 }}>No organs available</div>}
                            </div>
                        </div>
                        ...
                        <div className="panel">
                            ...
                            <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                                {[
                                    ...
                                    { label: 'Accepted', value: stats.offer_stats?.accepted_offers || 0, color: 'var(--teal)' },
                                    { label: 'Declined', value: stats.offer_stats?.declined_offers || 0, color: 'var(--coral)' },
                            ...
                            <tbody>
                                {donors.slice(0, 6).map(d => (
                                    <tr key={d.donor_id}>
                                        ...
                                        <td style={{ color: 'var(--text-3)', fontSize: 11 }}>{new Date(d.created_at).toLocaleDateString('en-IN')}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
                    {[
                        { label: 'Register Recipient', icon: '🏥', to: '/recipients/register', color: 'var(--blue)' },
                        { label: 'Waiting List', icon: '📋', to: '/waiting-list', color: 'var(--purple)' },
                        { label: 'Location Map', icon: '📍', to: '/map', color: 'var(--teal)' },
